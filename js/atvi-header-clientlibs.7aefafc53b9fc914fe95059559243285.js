...
MenubarItem.prototype.handleMouseout = function (event) {
  this.hasHover = false;
  if(this.popupMenu) setTimeout(this.popupMenu.close.bind(this.popupMenu, false), 10);
};
...
(function () {
  ...
  var initSSO = function () {
      ...
      if(sb) {
        ...
        $("#sso-login a").attr("href", ssobar.getLoginUrl());
        $("#sso-signup a").attr("href", ssobar.getSignupUrl());
        $("#sso-profile a").attr("href", ssobar.getCompleteProfileUrl());
        $("#sso-logout a").attr("href", ssobar.getLogoutUrl());
      }
  };
  ...
  var setRtl = function () {
    ...
    if (url.indexOf("/ar") > 0) $("html").attr("dir", "rtl");
  };
...
(function ($, ATVI) {
    ...
    ATVI.definePageView = function () {
        var d = digitalData.page,
            ...
            sectionStr = "hub",
            pageTypeStr = "home",
            pageNameStr = "activision:hub:home",
            i;
        if (url.indexOf("/games") > -1) {
            sectionStr = "games";
            siteSubsectionStr = "all games";
            pageTypeStr = "sub category";
            pageNameStr = "activision:games:all-games";
        }
        if (url.indexOf("/company/") > -1) {
            var companySubPath = url.split("/company/").slice(1);
            sectionStr = "company";
            pageTypeStr = "sub category";
            pageNameStr = "activision:company";
            siteSubsectionStr = companySubPath[0].replace("/", ":");
            pageNameStr += ":" + companySubPath[0].replace("/", ":");
        }
        obj.event = "pageView";
        obj.site = "activision";
        obj.siteSection = sectionStr;
    ...
    var initNavAnalytics = function () {
        document
            .querySelectorAll(".desktop-header a[data-label], .mobile-header a[data-label]")
            .forEach(function (node) {
                node.addEventListener("click", function (e) {
                    updateDataLayer($(this), this.dataset.label, "navigation", "main nav");
                });
            });
    };
